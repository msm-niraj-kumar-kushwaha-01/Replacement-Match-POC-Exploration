...
# === CONFIG ===
input_folder = r"C:\Users\user\Downloads\All_Timing_Belt_files"      # Update your input folder path here
output_folder = r"C:\Users\user\Downloads\transformed_folder"  # Update your output folder path here
...
def replace_circled_params(val, row, io_type):
    val = str(val)
    for i in range(1, 11):
        symbol = chr(9311 + i)  # ① starts at Unicode 9312 (9311+1)
        ...
        if irregular and irregular.lower() != 'nan':
            replacement = f'[{irregular}]{unit}'
        elif all(pd.notna(x) and str(x).strip() != '' for x in [min_val, max_val, range_val]):
            replacement = f'[{format_number(min_val)}-{format_number(max_val)}/{format_number(range_val)}]{unit}'
...
def write_batch(batch_data, batch_number):
    ...
    print(f"✅ Wrote batch {batch_number}: {output_path}")
        ...
        for _, row in df.iterrows():
            for io in ['input', 'output']:
                ...
                if pd.isna(part_number) or str(part_number).strip() == '':
                    continue
                part_number_cleaned = replace_circled_params(part_number, row, io)
...
print("🎉 All files processed and batches saved.")
